...
# DEVELOPER WARNING: There is a lot of redundancy between this file and the postinst script in the cryfs repository. Please port modifications to there!

set -e

DEBIAN_VERSIONS="jessie"
UBUNTU_VERSIONS="precise trusty xenial yakkety zesty"
...
add_repository () {
  ...
  sudo sh -c "echo \"deb $repo_url $DISTRIBUTION_VERSION main\" > $dir/cryfs.list"
}
...
install_key () {
  ...
  sudo apt-key add - > /dev/null <<KEYEND
